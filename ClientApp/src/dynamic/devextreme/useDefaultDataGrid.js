...
const useCustomGrid = (config: DataGridConfig) => {
    ...
    config.reference = config.reference || gridRef;
    ...
    const childs = useMemo(() =>  settings.childs(config.reference), [settings.childs]);
...
const getGridProps = (config: DataGridConfig): IDataGridOptions => {
    let {defaultValues, onEditingStart, childs, updateInterval, ignore, otherConfigs = {}, reference = {}, columnChooser = false, allowAdding = true, allowDeleting = true, allowUpdating = true, dataSource = [], columns, editorMode = 'cell'} = config;
